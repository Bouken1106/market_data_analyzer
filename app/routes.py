...
def _normalize_rebalance_frequency(raw: str) -> str:
    ...
    if key not in {"weekly", "monthly", "quarterly"}:
        raise HTTPException(
            status_code=400,
            detail="rebalance_frequency must be one of: weekly, monthly, quarterly.",
        )
    return key
...
def _build_trade_proposals(
    ...
    target_weight_map = {
        str(item.get("symbol") or "").upper().strip(): float(item.get("weight") or 0.0)
        for item in target_allocations
        if isinstance(item, dict)
    }
    current_symbols = set(pos_map.keys())
    target_symbols = set(target_weight_map.keys())
    all_symbols = sorted(current_symbols.union(target_symbols))

    proposals: list[dict[str, Any]] = []
    skipped: list[dict[str, Any]] = []

    for symbol in all_symbols:
        weight = target_weight_map.get(symbol, 0.0)
        target_value = max(0.0, portfolio_equity * weight)
        current_item = pos_map.get(symbol, {})
        current_value_raw = current_item.get("market_value")
        try:
        ...
        quantity = abs(delta_value) / float(price)
        proposals.append(
            {
                "symbol": symbol,
...
async def strategy_evaluate(req: StrategyEvaluationRequest) -> JSONResponse:
    ...
    latest_prices = {
        ...
        if isinstance(item, dict) and _to_valid_price(item.get("price")) is not None
    }
